...
/// </para>
/// <para>
/// Unlike the in-process <c>EdgeMap</c> which uses polymorphic <c>EdgeRunner</c> objects,
/// this implementation uses data-driven routing based on <see cref="WorkflowGraphInfo"/>.
/// </para>
/// </remarks>
internal sealed class DurableEdgeMap
{
    ...
    public DurableEdgeMap(WorkflowGraphInfo graphInfo)
    {
        ...
        ILogger logger)
    {
        ...
        DurableMessageEnvelope envelope = DurableMessageEnvelope.FromExecutor(sourceId, message, inputTypeName);
    ...
    public void EnqueueInput(
        ...
        Dictionary<string, Queue<DurableMessageEnvelope>> messageQueues)
    {
        DurableMessageEnvelope envelope = DurableMessageEnvelope.ForInput(message, inputTypeName);
    ...
    public bool IsFanInExecutor(string executorId)
    {
